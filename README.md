...
**Seamless Implicit/Explicit Trait Conversion (From / Into)**:
- Raw error types generated by `set!`, `union!`, or `#[derive(Error)]` automatically implement the marker trait `DiagnosticError`.
...
- **Chained Explicit Conversion (`to_report_trans`)**:
  - On `Result<T, E>`, you can use `.to_report_trans::<NewE>()` to lift and directly convert the inner error type to `NewE` (requires `E: Into<NewE>`).
  ...
  // Implicit promotion and conversion: AuthError -> Report<ApiError>
  let report: Report<ApiError> = AuthError::InvalidToken.into();

  // Automatic propagation and conversion with `?` (when returning Result<_, Report<ApiError>>):
